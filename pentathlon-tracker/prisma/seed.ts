...
async function main() {
  ...
  // ─── Stock Athlete User ─────────────────────────────────────────────────────
  const stockAthleteEmail = process.env.STOCK_ATHLETE_EMAIL || "athlete@pentathlon.ca";
  const stockAthletePassword = process.env.STOCK_ATHLETE_PASSWORD || "athlete123";
  const stockAthleteFirstName = process.env.STOCK_ATHLETE_FIRST_NAME || "Test";
  const stockAthleteLastName = process.env.STOCK_ATHLETE_LAST_NAME || "Athlete";

  const stockAthleteUser = await prisma.user.upsert({
    where: { email: stockAthleteEmail },
    update: { role: "athlete" }, // Always ensure this user stays athlete
    create: {
      name: `${stockAthleteFirstName} ${stockAthleteLastName}`,
      email: stockAthleteEmail,
      passwordHash: hashSync(stockAthletePassword, 10),
      role: "athlete",
    },
  });
  console.log("Stock athlete user:", stockAthleteUser.email);

  // ─── Stock Athlete Profile ──────────────────────────────────────────────────
  const stockAthlete = await prisma.athlete.upsert({
    where: { userId: stockAthleteUser.id },
    update: {
      firstName: stockAthleteFirstName,
      lastName: stockAthleteLastName,
      country: "CAN",
      ageCategory: "Senior",
      gender: "M",
      club: "Calgary AC",
    ...
    create: {
      ...
      gender: "M",
      club: "Calgary AC",
      userId: stockAthleteUser.id,
    },
  });
  console.log("Stock athlete profile:", `${stockAthlete.firstName} ${stockAthlete.lastName}`);
