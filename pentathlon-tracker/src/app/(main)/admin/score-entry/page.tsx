...
export default function ScoreEntryHubPage() {
  ...
  // Show active first, then upcoming, then completed
  ...
  return (
    <>
      ...
      <div className="max-w-[900px] mx-auto px-6 py-12">
        <h1 className="text-[28px] md:text-[40px] font-bold text-[#37352F] tracking-tight mb-4 leading-tight">
          Score Entry
        </h1>
        <p className="text-sm text-[#787774] mb-10">
          Select a competition to enter or edit scores.
        </p>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {sorted.map((comp) => {
              ...
              const inProgressEvents = comp.events.filter(
              ...
              return (
                <Link
                  key={comp.id}
                  href={`/admin/competitions/${comp.id}/score-entry`}
                  ...
                  <div>
                    ...
                    {inProgressEvents.length > 0 && (
                      <div className="text-xs text-[#0F7B6C] mt-1">
                        In progress:{" "}
                        {inProgressEvents
                          .map((e) => e.discipline.replace(/_/g, " "))
                          .join(", ")}
                      </div>
                    )}
                  </div>
                  <div className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-[#0B6E99] border border-[#0B6E99] rounded-[3px] opacity-0 group-hover:opacity-100 transition-opacity">
                    <PenLine size={12} />
                    Enter Scores
                  </div>
                </Link>
