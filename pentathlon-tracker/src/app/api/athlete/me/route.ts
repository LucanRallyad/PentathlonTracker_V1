...
/**
 * GET /api/athlete/me
 * Finds the Athlete record linked to the logged-in User,
 * then returns full competition history, scores, and computed stats.
 *
 * Link priority:
 * 1. Direct link via athlete.userId
 * 2. Name match (User.name ↔ Athlete firstName+lastName) — auto-links if found
 */
export async function GET(req: NextRequest) {
  ...
  if ("userId" in session) {
    ...
    // 2. Fallback: match by name and auto-link
    if (!athlete) {
      athlete = await findAndLinkByName(user.id, user.name);
    }
  }
  ...
  if (!athlete) {
    return NextResponse.json({
      athlete: null,
      user: userData,
      message: "No athlete record found linked to your account.",
    });
  }
  ...
  // Group training entries by discipline
...
// ── Helper: find athlete by name and auto-link ──

async function findAndLinkByName(userId: string, userName: string) {
