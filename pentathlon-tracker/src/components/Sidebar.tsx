        ...
        <div className="flex items-center justify-between px-3 py-3 border-b border-[#E9E9E7]">
          ...
          {isMobile ? (
            <button
              ...
              className="p-2.5 rounded-[3px] hover:bg-[#EFEFEF] text-[#787774] transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center"
            >
              <X size={20} />
            </button>
          ) : (
            ...
            <>
              ...
              {!isCollapsed && (
                <div className="px-4 mb-1">
                  <span className="text-[11px] md:text-[10px] font-medium text-[#9B9A97] uppercase tracking-wider">
                    My Account
                  </span>
                </div>
              )}
        ...
        {!loaded && isLoggedIn && (
          <div className="border-t border-[#E9E9E7] px-2 py-2">
            <div className={cn("flex items-center gap-2 px-2 py-1.5", isCollapsed && "justify-center")}>
              <div className="w-7 h-7 md:w-6 md:h-6 rounded-full bg-[#0B6E99] flex items-center justify-center text-white text-xs md:text-[10px] font-bold flex-shrink-0">
                {user.name?.charAt(0)?.toUpperCase() || "?"}
              </div>
              {!isCollapsed && (
                <div className="min-w-0">
                  <div className="text-sm md:text-xs font-medium text-[#37352F] truncate">{user.name}</div>
                  <div className="text-xs md:text-[10px] text-[#9B9A97] capitalize">{user.role?.replace('_', ' ')}</div>
                </div>
              )}
            </div>
          </div>
...
const NavItem = memo(function NavItem({
  ...
  return (
    <Link
      href={item.href}
      className={cn(
        "flex items-center gap-2.5 px-2 py-2.5 md:py-1.5 rounded-[3px] text-sm transition-colors duration-150 min-h-[44px] md:min-h-0",
        active
