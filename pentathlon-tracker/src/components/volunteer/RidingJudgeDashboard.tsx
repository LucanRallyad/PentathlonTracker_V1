...
function CounterRow({
  ...
  const btnSize = isDesktop ? "w-14 h-14 min-w-[56px] min-h-[56px]" : "w-12 h-12 min-w-[48px] min-h-[48px]";
  return (
    <div className="flex items-center justify-between bg-white rounded-[4px] border border-[#E9E9E7] px-4 py-3">
      ...
      <div className="flex items-center gap-3">
        <button
          onClick={onDec}
          disabled={value <= 0}
          className={`${btnSize} rounded-full bg-[#F7F6F3] active:bg-[#E8E7E4] border border-[#E9E9E7] text-xl font-bold text-[#37352F] disabled:opacity-30`}
        >
          &minus;
        </button>
        ...
        <button
          onClick={onInc}
          className={`${btnSize} rounded-full bg-[#F7F6F3] active:bg-[#E8E7E4] border border-[#E9E9E7] text-xl font-bold text-[#37352F]`}
        >
          +
        </button>
      </div>
...
export default function RidingJudgeDashboard({
  ...
  const scoreColorClass = calculatedPoints < 200 ? "text-[#E03E3E]" : calculatedPoints < 280 ? "text-[#D9730D]" : "text-[#0F7B6C]";

  const penaltyCounters = (
  ...
  const penaltySummary = (
    <div className="bg-[#F7F6F3] rounded-[4px] border border-[#E9E9E7] px-4 py-3">
      ...
      <div className="flex justify-between text-sm mt-1">
        ...
        <span className="text-[#D9730D] font-bold font-mono">{calculatedPoints} / 300</span>
      </div>
    </div>
  );
  ...
  const courseTimer = (
    <div className="bg-white rounded-[4px] border border-[#E9E9E7] p-4 text-center">
      <p className="text-xs text-[#9B9A97] uppercase tracking-wider mb-1">Course Time</p>
      <p
        className={`font-mono font-bold ${isDesktop ? "text-5xl" : "text-3xl"} ${
      ...
      <div className="flex gap-3 mt-3 justify-center">
        {timerState === "idle" && (
          <button
            onClick={handleStart}
            className={`px-8 py-2 rounded-[4px] bg-[#0F7B6C] active:bg-[#0a6358] text-white font-semibold uppercase text-sm ${isDesktop ? "min-h-[40px]" : "min-h-[48px]"}`}
          >
            Start
          </button>
        )}
        {timerState === "running" && (
          <button
            onClick={handleStop}
            className={`px-8 py-2 rounded-[4px] bg-[#E03E3E] active:bg-[#c43333] text-white font-semibold uppercase text-sm ${isDesktop ? "min-h-[40px]" : "min-h-[48px]"}`}
          >
            Stop
          </button>
        )}
        {timerState === "stopped" && (
          <button
            onClick={handleStart}
            className={`px-6 py-2 rounded-[4px] bg-[#F7F6F3] active:bg-[#E8E7E4] text-[#37352F] border border-[#E9E9E7] font-semibold text-sm ${isDesktop ? "min-h-[40px]" : "min-h-[48px]"}`}
          >
            Restart
          </button>
        )}
  ...
  if (isDesktop) {
    return (
      <div className="flex flex-row min-h-[600px] bg-[#FBFBFA] text-[#37352F] rounded-[4px] border border-[#E9E9E7] overflow-hidden">
        {/* Left — Timer + Score Summary + Confirm */}
          ...
          <header className="bg-white px-6 py-4 border-b border-[#E9E9E7]">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <p className={`text-3xl font-bold font-mono ${scoreColorClass}`}>{calculatedPoints}</p>
          ...
          <div className="flex-1 flex flex-col p-6 gap-4">
            ...
            <div className="mt-auto">
              <button
                ...
                className="w-full py-4 rounded-[4px] bg-[#0F7B6C] active:bg-[#0a6358] disabled:bg-[#F7F6F3] disabled:text-[#9B9A97] disabled:border disabled:border-[#E9E9E7] text-white text-base font-semibold uppercase min-h-[48px]"
              >
                Confirm Score
              </button>
            </div>
    ...
    <div className="flex flex-col min-h-screen bg-[#FBFBFA] text-[#37352F]">
      <header className="bg-white px-4 py-3 border-b border-[#E9E9E7]">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <p className={`text-2xl font-bold font-mono ${scoreColorClass}`}>{calculatedPoints}</p>
            <p className="text-xs text-[#9B9A97]">/ 300</p>
          </div>
        </div>
      </header>

      <div className="px-4 pt-4 pb-2">{courseTimer}</div>

      <div className="flex-1 px-4 space-y-2 overflow-y-auto pb-2">
        <p className="text-xs text-[#9B9A97] uppercase tracking-wider mt-2">Penalties</p>
        {penaltyCounters}
        {penaltySummary}
      </div>
