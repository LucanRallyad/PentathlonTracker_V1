...
const AGE_CATEGORIES = ["All", "Senior", "Junior", "U19", "U17", "U15"];
...
export default function RankingsPage() {
  ...
  const showStandardRef = ageCategoryFilter !== "All";
  ...
  return (
    <>
      ...
      <div className="max-w-[1200px] mx-auto px-6 py-8">
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-[32px] font-bold text-[#37352F] tracking-tight leading-tight flex items-center gap-3">
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {AGE_CATEGORIES.map((cat) => (
              <button
                key={cat}
                onClick={() => setAgeCategoryFilter(cat)}
                className={`px-3 py-1.5 text-xs font-medium rounded-[4px] border transition-colors ${
        ...
        {ranked.length === 0 ? (
          <div className="text-center py-16 border border-dashed border-[#E9E9E7] rounded-[4px]">
            ...
            <p className="text-sm text-[#9B9A97]">No rankings available for the selected filters.</p>
